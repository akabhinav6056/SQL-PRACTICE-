...
left  join student s on c.course_id = s.student_course_id ) as test1 where student_id is null 
...
select * from course1 where course_id = 108
...
select course_id  from course union select student_id from student

select course_id  from course union all select student_id from student

with sample_studnets as (
select * from course where course_id in (102,103,104,106,101))
select * from sample_studnets where course_tag = 'java '  
