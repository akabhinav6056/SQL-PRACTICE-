...
select  * from bank_details where age = 60 or job='retired' 

select * from bank_details where (education = 'unknown' or marital ='single') and (balance between 0 and 500 )
...
END  &&
 ...
 DELIMITER &&
 create procedure recbyjob( IN input varchar(30))
 begin 
 select * from bank_details where job = input  ;
 end &&
 
 call recbyjob('admin.')
 
 create view mostused as select age , job , marital , education , balance from bank_details ;
 
 select * from mostused 
 
 DELIMITER &&
 create procedure Job_balance(in v1 int )
 begin 
 select * from bank_details where  balance <v1 ;
 select job from bank_details where balance <v1;
 end &&
